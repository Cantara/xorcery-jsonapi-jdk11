...
public class VariableResolver
    ...
    private ObjectNode resolveObject(ObjectNode source, ObjectNode node) {
        ...
        while (names.hasNext()) {
            ...
            if (value instanceof TextNode) {
                ...
                result.set(next, resolveObject(source, (ObjectNode) value));
            } else if (value instanceof ArrayNode) {
                result.set(next, resolveArray(source, (ArrayNode) value));
            } else {
    ...
    private ArrayNode resolveArray(ObjectNode source, ArrayNode node) {
        ...
        for (JsonNode value : node) {
            if (value instanceof TextNode) {
                ...
                result.add(resolveObject(source, (ObjectNode) value));
            } else if (value instanceof ArrayNode) {
                result.add(resolveArray(source, (ArrayNode) value));
            } else {
                result.add(value);
            }
        }
        ...
        return lookup(source, name)
                .map(result ->
                {
                    if (result instanceof TextNode) {
                        return resolveValue(source, (TextNode) result);
                    } else if (result instanceof ObjectNode) {
                        return resolveObject(source, (ObjectNode) result);
                    } else if (result instanceof ArrayNode) {
                        return resolveArray(source, (ArrayNode) result);
                    } else {
                        return result;
                    }
                });
    ...
    private Optional<JsonNode> lookup(ObjectNode source, String name) {
        ...
        for (int i = 0; i < names.length - 1; i++) {
            ...
            {
                node = resolveValue(source, (TextNode) node);
            }
